...
function JoinGroupPopup({setIsGroupJoinPopupOpen}) {
    ...
    useEffect(() => {
      const main  =  async  ()  => {
        try{
          const groupsListResponse = await getGroupsList(localStorage.getItem('token'),localStorage.getItem('user_name'));
    ...
    const  handleInputChangeEvent = (e) => {
        const newFilteredData   =  groupsList.filter((val) => {
            if(val.name.toLowerCase().includes(e.target.value.trim().toLowerCase()) || e.target.value == '')  return true;
            return false
        })
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
              ...
              <button
                ...
                onClick={() =>{setIsGroupJoinPopupOpen(false)}}
              >
                <svg
                  className="w-3 h-3"
                  aria-hidden="true"
              ...
              <form className="space-y-4" onSubmit={(ev) => {ev.preventDefault()}}>
                ...
                <div className="friendList border p-2  rounded-md border-gray-600 bg-gray-600 flex flex-col">
                  ...
                  <ul className="h-48 overflow-y-auto scrollbar-hidden">
                    {filterGroups.map((val,index)=>  <GroupsList data={val} key={index} />)}
                    {filterGroups.length == 0 &&  <p  className="font-light italic text-sm">No  Group  Found...</p>}
                  </ul>
                </div>
              </form>
            </div>
