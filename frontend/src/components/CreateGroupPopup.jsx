...
import { createNewGroup } from "../helpers/groupHelpers";
...
function CreateGroupPopup({ setIsGroupCreatePoupOpen }) {
  ...
  useEffect(() => {
    const main = async () => {
      const friendsListResponse = await getUsersFriends();
  ...
  const handleFormSubmit = async (ev) => {
    ...
    const response = await createNewGroup(groupName, members);
    if (!response.status) {
      ...
      setTimeout(() => {
        setIsGroupCreatePoupOpen(false);
      }, 1500);
    }
  };
