...
import {manageGroupRequestsByUser} from   "../api/group";
...
function GroupRow({isRequestRow  =  false,data}) {
  const  handleGroupRequest = async (action) =>  {
    try{
      const  groupRequestResponse =  await manageGroupRequestsByUser(localStorage.getItem('token'),localStorage.getItem('user_name'),{
          ...
          <div className="rightSection  w-full  pt-2 pr-4">
            <div className="twoSection  flex">
              <div className="leftSection w-full">
                ...
                <h3 className="font-semibold text-sm">Admin:  {data?.adminUsername}</h3>
              </div>
              <div className={isRequestRow ? "rightSection w-1/3"  : "rightSection w-20"}>
                {!isRequestRow   && <p className="text-xs">Active</p>}
                {isRequestRow &&   <div>
                    <button className='bg-green-600   hover:bg-green-700  cursor-pointer  text-xs font-semibold mr-2 px-2   py-1  rounded-md'  onClick={() => {handleGroupRequest('accept')}}>Accept  Invite</button>
                    <button className='bg-red-600   hover:bg-red-700  cursor-pointer  text-xs font-semibold mr-2 px-2   py-1  rounded-md' onClick={() => {handleGroupRequest('rejected')}}>Reject   Invite</button>
                    </div>}
              </div>
            </div>
          </div>
