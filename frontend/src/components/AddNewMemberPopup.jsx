...
import { getUsersListForAddingNewMembers } from "../helpers/userHelpers";
...
function AddNewMemberPopup({ setIsAddNewMemberPopupOpen, groupName }) {
  ...
  useEffect(() => {
    const main = async () => {
      const response = await getUsersListForAddingNewMembers(groupName);
      if (response.status) {
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            ...
            <button
              ...
              onClick={() => {
                setIsAddNewMemberPopupOpen(false);
              }}
            >
              <svg
                className="w-3 h-3"
